(function(s,o){typeof exports=="object"&&typeof module<"u"?o(exports):typeof define=="function"&&define.amd?define(["exports"],o):(s=typeof globalThis<"u"?globalThis:s||self,o(s.Eddystrap={}))})(this,function(s){"use strict";class o{constructor(t={}){this.commitment=0,this.isActive=!1,this.startTime=null,this.animationFrame=null,this.options={color:"#6366f1",size:200,activationTime:500,onActivate:null,label:"Eddy",sublabel:"Touch & hold",...t},this.element=document.createElement("div"),this.progressCircle=document.createElementNS("http://www.w3.org/2000/svg","circle"),this.create()}create(){this.element.style.cssText=`
    ...
    `,this.element.appendChild(e),this.attachEvents(),this.updateStyle()}attachEvents(){this.element.addEventListener("mousedown",t=>this.start(t)),this.element.addEventListener("mouseup",()=>this.end()),this.element.addEventListener("mouseleave",()=>this.end()),this.element.addEventListener("touchstart",t=>{t.preventDefault(),this.start(t)},{passive:!1}),this.element.addEventListener("touchend",t=>{t.preventDefault(),this.end()},{passive:!1}),this.element.addEventListener("touchcancel",()=>this.end())}start(t){this.startTime=Date.now(),this.isActive=!0,this.element.style.transform="scale(1.1)",this.animate(),this.createRipple(t)}animate(){if(!this.isActive||this.startTime===null)return;const t=Date.now()-this.startTime;if(this.commitment=Math.min(t/this.options.activationTime*100,100),this.updateStyle(),this.commitment>=100){this.activate();return}this.animationFrame=requestAnimationFrame(()=>this.animate())}end(){this.isActive=!1,this.commitment=0,this.startTime=null,this.element.style.transform="scale(1)",this.updateStyle(),this.animationFrame!==null&&(cancelAnimationFrame(this.animationFrame),this.animationFrame=null)}activate(){this.options.onActivate&&this.options.onActivate();for(let t=0;t<6;t++)setTimeout(()=>{const e=t/6*360,i=document.createElement("div");i.style.cssText=`
        ...
        `,this.element.appendChild(i),setTimeout(()=>i.remove(),600)},t*50);this.end()}createRipple(t){const e=this.element.getBoundingClientRect();let i,n;if("touches"in t&&t.touches[0])i=(t.touches[0].clientX-e.left)/e.width*100,n=(t.touches[0].clientY-e.top)/e.height*100;else if("clientX"in t)i=(t.clientX-e.left)/e.width*100,n=(t.clientY-e.top)/e.height*100;else return;const r=document.createElement("div");r.style.cssText=`
      position: absolute;
      left: ${i}%;
      top: ${n}%;
      width: 30px;
      ...
      animation: flowRipple 0.6s ease-out;
    `,this.element.appendChild(r),setTimeout(()=>r.remove(),600)}updateStyle(){const t=this.commitment/100*25,e=.5+this.commitment/100*.5;this.element.style.background=`radial-gradient(circle, ${this.options.color}50, ${this.options.color}20)`,this.element.style.border=`2px solid ${this.options.color}${Math.round(e*255).toString(16).padStart(2,"0")}`,this.element.style.boxShadow=`
      0 0 ${t}px ${this.options.color}90,
      inset 0 0 ${t*.5}px ${this.options.color}50
    `;const i=this.options.size*.45,n=2*Math.PI*i,r=`${this.commitment/100*n}, ${n}`;this.progressCircle.style.strokeDasharray=r,this.progressCircle.style.stroke=this.options.color,this.progressCircle.style.filter=`drop-shadow(0 0 ${t*.5}px ${this.options.color})`}mount(t){const e=typeof t=="string"?document.querySelector(t):t;if(!e)throw new Error(`Target element not found: ${t}`);return e.appendChild(this.element),this}destroy(){return this.animationFrame!==null&&cancelAnimationFrame(this.animationFrame),this.element&&this.element.parentNode&&this.element.parentNode.removeChild(this.element),this}}class l{constructor(t={}){this.options={color:"#6366f1",content:"",onDrain:null,...t},this.element=document.createElement("div"),this.create()}create(){this.element.className="flow-pool",this.element.style.cssText=`
      position: fixed;
      inset: 0;
    ...
    `,i.addEventListener("mouseenter",()=>{i.style.background=`${this.options.color}40`,i.style.borderColor=`${this.options.color}80`}),i.addEventListener("mouseleave",()=>{i.style.background=`${this.options.color}26`,i.style.borderColor=`${this.options.color}4d`}),i.addEventListener("click",()=>this.drain()),t.appendChild(i),this.element.appendChild(t),this.element.addEventListener("click",n=>{n.target===this.element&&this.drain()})}enter(){return this.element.style.display="flex",this}drain(){return this.element.style.display="none",this.options.onDrain&&this.options.onDrain(),this}mount(t){const e=typeof t=="string"?document.querySelector(t):t;if(!e)throw new Error(`Target element not found: ${t}`);return e.appendChild(this.element),this}destroy(){return this.element&&this.element.parentNode&&this.element.parentNode.removeChild(this.element),this}}const a={Eddy:o,Pool:l};s.Eddy=o,s.Pool=l,s.default=a,Object.defineProperties(s,{__esModule:{value:!0},[Symbol.toStringTag]:{value:"Module"}})});
